...
Directory::Update() {
  ...
  if (!fromfile) {
    ...
    fobj.SetName(dirname);

    if (lastupdate == 0 || fobj.GetCtime() > lastupdate) {
      lastupdate = fobj.GetCtime();
      
      /* Clear list */
      list.clear();
      
      /* Load file */
      f.open(dirname.c_str(), ios::in);
      if (f.is_open()) {
	while (f) {
...
// PUBLIC
void
Directory::SetDirname(const string &_dirname, bool _fromfile) {
  dirname = _dirname;
  fromfile = _fromfile;
